...
class ProductModel extends Connect
{
    ...
    {
        $sql = "SELECT id, name, image, price, price_sale, status, sold_count FROM products WHERE status = 1 ORDER BY sold_count DESC LIMIT 4";
    ...
    public function getNewProduct(){
        $sql = "SELECT id, name, image, price, price_sale, status, sold_count FROM products WHERE status = 1 ORDER BY created_at DESC LIMIT 10";
        $data = $this->getAll($sql);
        $products = [];
        foreach ($data as $row) {
            $products[] = new ProductModel($row);
